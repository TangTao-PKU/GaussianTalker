...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    if opt.dataloader:
        viewpoint_stack = scene.getTrainCameras()     # the same with train_cams
        if opt.custom_sampler is not None:
            sampler = FineSampler(viewpoint_stack)
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size,sampler=sampler,num_workers=2,collate_fn=list, drop_last = True)
            random_loader = False
        else:
            viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=batch_size,shuffle=True,num_workers=2,collate_fn=list, drop_last = True)
            random_loader = True
        loader = iter(viewpoint_stack_loader)         # iter items
    ...
    for iteration in range(first_iter, final_iter+1):   # 여기부터 iteration 시작   
        ...
        if opt.dataloader and not load_in_memory:
            try:
                ...
                if not random_loader:
                    viewpoint_stack_loader = DataLoader(viewpoint_stack, batch_size=opt.batch_size,shuffle=True,num_workers=2,collate_fn=list)
                    random_loader = True
        ...
        #     pic = toPIL(image_tensor[0])
        #     pic.save('/data0/tangt/zpingan-Intern/GaussianTalker/data/hehe0521/test.jpg')
        #     print(iteration)
        ...
        if opt.hair_fine_tuning:
            hair_perceptual_loss = vgg_perceptual_loss(output["rendered_hair_tensor"],output["gt_hair_tensor"])*0.1
            loss+=hair_perceptual_loss
        # print_test_images
        # if (iteration %10 ==0 ): 
        #     toPIL = transforms.ToPILImage() 
        #     pic = toPIL(output["rendered_hair_tensor"][0])
        #     pic.save('/data0/tangt/zpingan-Intern/GaussianTalker/data/hehe0521/render_hair.jpg')
        #     pic = toPIL(output["gt_hair_tensor"][0])
        #     pic.save('/data0/tangt/zpingan-Intern/GaussianTalker/data/hehe0521/gt_hair.jpg')
            # print(iteration)
        ...
        with torch.no_grad():
            ...
            if iteration % 500 == 0 or iteration == 1:
                ...
                scene.save(iteration, stage, torch.cat([gt_image_tensor, image_tensor, output["rendered_hair_tensor"],output["gt_hair_tensor"]]), viewpoint_cams[0].uid)
