    ...
    def __getitem__(self, index):
        if self.dataset_type != "PanopticSports":
            ...
            if seg is None:
                ...
                # (450, 450, 3)  mask_path = cam_name.replace('ori_imgs', 'parsing').replace('.jpg', '.png')
            head_mask = (seg[..., 0] == 255) & (seg[..., 1] == 0) & (seg[..., 2] == 0)      # (450, 450)
            bg_w_torso = torso_image[:3,...] * torso_image[3:,...] + bg_image * (1-torso_image[3:,...])

            # 2type
            hair_mask = cv2.imread(caminfo.mask_path)      # np.array([60, 179, 113])
            hair_mask = (hair_mask[..., 0] == 255) & (hair_mask[..., 1] == 0) & (hair_mask[..., 2] == 0)
            ...
            # pic = toPIL(full_image)
            # pic.save('/data0/tangt/zpingan-Intern/GaussianTalker/data/Obama/test.jpg')
            
            return Camera(colmap_id=index,R=R,T=T,FoVx=FovX,FoVy=FovY,gt_image=full_image, head_mask=head_mask,hair_mask=hair_mask, bg_image = bg_image,
