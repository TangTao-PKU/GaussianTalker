...
# def yaw_control(poses, degree, id):
# opt 로 novelview angle 받아야 될듯 , id 는 아나 ? 
# id = self.opt.id
# id.lowercase()

def yaw_control(poses, degree, id, offset = True):
    rad = torch.tensor(degree * math.pi / 180.0)
    # yaw rot
    yaw_rot = torch.tensor([
    [torch.cos(rad), 0, torch.sin(rad)],
    [0, 1, 0],
    [-torch.sin(rad), 0, torch.cos(rad)]
    ])

    yaw_homo_mtx = torch.eye(4) 
    yaw_homo_mtx[:3, :3] = yaw_rot  
    yaw_mtx = yaw_homo_mtx.repeat(poses.shape[0], 1, 1)

    first_pose = poses[0]

    # headonly
    id_offset = {
                "obama1" : { 30 : torch.tensor([0.04, 0.025, +0.000], dtype=torch.float64) , -30 : torch.tensor([-0.03, 0.025, +0.000], dtype=torch.float64)} ,
                "may": { 30 :torch.tensor([0.03, 0.018, +0.000], dtype=torch.float64), -30 : torch.tensor([-0.03, 0.018, +0.000], dtype=torch.float64) }
                }
    # comp
    id_offset = {
                "obama1" : { 30 : torch.tensor([0.04, 0.00, +0.000], dtype=torch.float64) , -30 : torch.tensor([-0.03, +0.00, +0.000], dtype=torch.float64)} ,
                "may": { 30 :torch.tensor([0.03, 0.018, +0.000], dtype=torch.float64), -30 : torch.tensor([-0.03, 0.018, +0.000], dtype=torch.float64) }
                }
    
    if offset:
        trans_offset = id_offset[id][degree]
        
    # wo offset 
    else :
        trans_offset = torch.tensor([0,0,0], dtype=torch.float64)

    first_pose[:3, 3] += trans_offset
    first_pose = nerf_matrix_to_ngp(first_pose, scale=self.scale, offset=self.offset)

    first_poses = first_pose.repeat(poses.shape[0], 1, 1)
    rotated = torch.bmm(yaw_mtx, first_poses)
